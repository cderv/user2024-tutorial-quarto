...
::: aside
Full version at: <https://events.linuxfoundation.org/user/attend/code-of-conduct/>
:::
...
-   **Instructeur:** Christophe Dervieux
    ...
    -   Working on R Markdown et now also member of Quarto core dev team.
...
- During non-exercise parts, or in transitions - raise your hand {{< fa hand >}}

- During the exercises -- {{< fa hand >}}, but also use the stickers to signal you :

    - [Pink]{.fg style="--col: pink"}: I need help!
    - [Blue]{.fg style="--col: blue"}: I'm done!
...
quarto render document.qmd # defaults to `format` key in document (or html)
...
How it works => The **quarto** R {{< fa box >}} call `quarto` CLI from R through **processx** package.
...
3. Create a new Quarto document - you can call it `simple-document.qmd`.
4. Click on "Render".
5. Open visual editor
:::

::: callout-tip
`simple-document.qmd` is available for [download](/1-quarto-kesako/#exercises) if required
:::
